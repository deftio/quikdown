...
/**
 * Simplified EBNF Compiler for QuikDown
 * Directly generates optimized lexer from EBNF rules
 */
...
// Import styles for inlining
import { styles } from './quikdown_lex_styles.js';

/**
 * Generate the complete lexer code from EBNF-inspired rules
 * This is a hybrid approach: EBNF-style rules compiled to optimized JavaScript
 */
...
async function build() {
  console.log('🔨 Building EBNF-based quikdown_lex (simplified)...');
  
  try {
    // Generate the lexer code
    console.log('  📝 Generating EBNF-based lexer...');
    let code = generateLexer();
    ...
    fs.writeFileSync(path.join(__dirname, 'quikdown_lex_ebnf.js'), code);
    ...
    fs.writeFileSync(path.join(__dirname, 'quikdown_lex_ebnf.min.js'), minified.code);
    
    // Compare sizes
    console.log('\n📊 Size Comparison:');
    console.log(`  EBNF Source: ${(code.length / 1024).toFixed(1)}KB`);
    console.log(`  EBNF Minified: ${(minified.code.length / 1024).toFixed(1)}KB`);
    ...
    if (fs.existsSync(grammar1MinPath)) {
      const grammar1Size = (fs.statSync(grammar1MinPath).size / 1024).toFixed(1);
      console.log(`  Grammar1 Minified: ${grammar1Size}KB`);
      
      const diff = ((minified.code.length / 1024 - parseFloat(grammar1Size)) / parseFloat(grammar1Size) * 100).toFixed(1);
