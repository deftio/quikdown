...
try {
  ...
  const modifiedTest = mainTestContent
    .replace(/import quikdown from ['"]\.\.\/src\/quikdown\.js['"]/g, 
             `import quikdown from '${path.join(__dirname, 'quikdown_lex_compact.js')}'`)
    .replace(/import quikdown from ['"]\.\.\/dist\/quikdown\.esm\.js['"]/g,
             `import quikdown from '${path.join(__dirname, 'quikdown_lex_compact.js')}'`)
    .replace(/const quikdown = require\(['"]\.\.\/src\/quikdown['"]\)/g,
             `const quikdown = require('${path.join(__dirname, 'quikdown_lex_compact.js')}')`);
  ...
  const { default: quikdown_lex } = await import('./quikdown_lex_compact.js');
