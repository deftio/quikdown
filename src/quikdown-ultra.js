...
function quikdown(md,opt={}){
  ...
  const I=(t)=>{
    ...
    // Italic - fixed for single asterisk
  ...
  while(i<lines.length){
    ...
    if(m){
      ...
      if(!list){
        list={tag,indent,html:`<${tag}${A(tag)}>`,stack:[{tag,indent}]};
      }else if(indent>list.stack[list.stack.length-1].indent){
        // Start nested list
        list.html+=`<${tag}${A(tag)}>`;
        list.stack.push({tag,indent});
      }else if(indent<list.stack[list.stack.length-1].indent){
        // Close nested lists
        while(list.stack.length>1&&indent<list.stack[list.stack.length-1].indent){
          const prev=list.stack.pop();
          list.html+=`</${prev.tag}>`;
        }
      }else if(tag!==list.stack[list.stack.length-1].tag){
  ...
  if(list){
    while(list.stack.length>0){
      const item=list.stack.pop();
      list.html+=`</${item.tag}>`;
    }
    o.push(list.html);
  }
